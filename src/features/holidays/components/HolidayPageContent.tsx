...
export default function HolidayPageContent({
  ...
  const getContextIcon = () => {
    if (regionCode) {
      return <CompassIcon className="h-5 w-5 text-teal-600" />;
    } else if (stateCode) {
      return <MapIcon className="h-5 w-5 text-teal-600" />;
    } else {
  ...
  return (
    <div className="container mx-auto py-6 px-4 max-w-5xl" role="main" aria-labelledby="page-title">
      ...
      <div className="bg-white rounded-lg shadow-sm mb-5 p-3 sm:p-4" role="banner">
        <div className="flex flex-wrap items-center justify-between gap-2 mb-3">
          <Link
            href={getParentRoute()}
            className="inline-flex items-center text-sm text-gray-500 hover:text-teal-600 py-1.5 px-2 -ml-2 rounded-md"
          ...
          <div className="flex items-center">
            <Badge variant="outline" className="px-2.5 py-1 text-xs flex items-center">
              <CalendarIcon className="h-3.5 w-3.5 mr-1.5 text-teal-600" aria-hidden="true" />
              <span>
        ...
        <div className="flex flex-col sm:flex-row sm:items-start gap-3">
          <div
            className="p-2 bg-teal-50 rounded-md flex-shrink-0 hidden sm:block"
            aria-hidden="true"
          >
            {getContextIcon()}
          </div>
          <div className="flex-1">
            <div className="flex items-center gap-2 mb-2">
              <div className="sm:hidden p-1.5 bg-teal-50 rounded-md mr-1" aria-hidden="true">
                {getContextIcon()}
              </div>
              <h1
                id="page-title"
            ...
            <div className="flex flex-wrap gap-2 mb-3">
              <Badge variant="outline" className="px-2.5 py-1 text-xs flex items-center">
                <GlobeIcon className="h-3.5 w-3.5 mr-1.5 text-teal-600" aria-hidden="true" />
            ...
            <div className="flex flex-wrap gap-x-4 gap-y-2 mt-3 pt-3 border-t border-gray-100">
              <div className="flex items-center min-w-[120px]">
                <CalendarDaysIcon className="h-4 w-4 mr-2 text-teal-600" aria-hidden="true" />
              ...
              {countryCode && !stateCode && states.length > 0 && (
                <div className="flex items-center min-w-[120px]">
                  <MapPinIcon className="h-4 w-4 mr-2 text-teal-600" aria-hidden="true" />
                  <span className="text-sm text-gray-600">
      ...
      {countryCode && !stateCode && states.length > 0 && (
        <div
          className="mb-5 bg-white rounded-lg shadow-sm overflow-hidden border-l-4 border-l-teal-500"
          role="region"
          aria-labelledby="states-heading"
        >
          <div className="px-4 py-3 border-b border-gray-100 bg-gradient-to-r from-teal-50 to-transparent flex items-center justify-between">
            <div className="flex items-center">
              <MapPinIcon className="h-4 w-4 mr-2 text-teal-600" aria-hidden="true" />
              <h2 id="states-heading" className="text-sm font-medium text-gray-900">
                Narrow By State/Province
              </h2>
            </div>
            <Badge variant="outline" className="bg-teal-50 text-teal-700 border-0 text-xs">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">
              {isLoadingStates
                ? Array(6)
                    .fill(0)
                    .map((_, i) => (
                      <div key={i} className="p-2 bg-gray-50 animate-pulse h-10 rounded"></div>
                    ))
                : states.map(state => (
                    <Link
                      key={state.code}
                      href={`/holidays/${countryCode?.toLowerCase()}/${state.code.toLowerCase()}`}
                      className="p-2.5 hover:bg-gray-50 rounded-md border border-gray-200 flex items-center group transition-colors"
                    >
                      <div
                        className="w-7 h-7 rounded-full bg-teal-50 flex items-center justify-center mr-2.5 flex-shrink-0"
                        aria-hidden="true"
                      >
                        <MapPinIcon className="h-3.5 w-3.5 text-teal-600" />
                      </div>
                      <span className="text-sm text-gray-800 group-hover:text-teal-600 truncate flex-1 font-medium">
                        {state.name}
                      </span>
                      <ChevronRightIcon
      ...
      {countryCode && stateCode && !regionCode && regions.length > 0 && (
        ...
        >
          <div className="px-4 py-3 border-b border-gray-100 bg-gradient-to-r from-teal-50 to-transparent flex items-center justify-between">
            <div className="flex items-center">
              <CompassIcon className="h-4 w-4 mr-2 text-teal-600" aria-hidden="true" />
              <h2 id="regions-heading" className="text-sm font-medium text-gray-900">
                Narrow By Region
              </h2>
            </div>
            <Badge variant="outline" className="bg-teal-50 text-teal-700 border-0 text-xs">
                ...
                : regions.map(region => (
                    ...
                    >
                      <div
                        ...
                        aria-hidden="true"
                      >
                        <CompassIcon className="h-3.5 w-3.5 text-teal-600" />
                      </div>
                      <span className="text-sm text-gray-800 group-hover:text-teal-600 truncate flex-1 font-medium">
                        {region.name}
                      </span>
      ...
      {getUpcomingHolidays().length > 0 && (
        <div
          className="mb-5 bg-white rounded-lg shadow-sm overflow-hidden border border-gray-100"
          role="region"
          aria-labelledby="upcoming-holidays-heading"
        >
          <div className="px-4 py-3 bg-gradient-to-r from-teal-50 to-transparent border-b border-gray-100 flex items-center">
            <CalendarDaysIcon className="h-4 w-4 mr-2 text-teal-600" aria-hidden="true" />
            <h2 id="upcoming-holidays-heading" className="text-sm font-medium text-gray-900">
              Upcoming Holidays
            </h2>
          </div>
          ...
          <ul className="divide-y divide-gray-100" role="list">
            {getUpcomingHolidays().map((holiday, index) => {
              ...
              let proximityColor = 'text-teal-600';
              if (daysUntil <= 7) {
                proximityColor = 'text-amber-600';
              }
              if (daysUntil <= 1) {
                proximityColor = 'text-red-600';
              }

              return (
                <li key={index} className="hover:bg-gray-50 transition-colors" role="listitem">
                  <div className="flex items-center px-4 py-3">
                    {/* Calendar-style date indicator */}
                    <div className="flex-shrink-0 mr-4">
                      <div className="w-14 h-14 rounded-lg overflow-hidden shadow-sm border border-gray-200 flex flex-col">
                        <div className="bg-teal-600 text-white text-xs font-medium py-0.5 text-center">
                          {month}
                        </div>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        <time dateTime={holiday.date} className="text-xs text-gray-500">
                          {formattedDate}
                        </time>
                        <span
      ...
      <div
        className="mb-5 bg-white rounded-lg shadow-sm overflow-hidden"
        role="region"
        aria-labelledby="holiday-calendar-heading"
      >
        <div className="px-4 py-3 border-b border-gray-100 flex items-center">
          <CalendarIcon className="h-4 w-4 mr-2 text-teal-600" aria-hidden="true" />
          <h2 id="holiday-calendar-heading" className="text-sm font-medium text-gray-900">
            Holiday Calendar
          </h2>
        </div>
        ...
        <div className="p-4">
          <Tabs defaultValue={currentYear.toString()} className="w-full">
            <TabsList className="w-full flex rounded-md border border-gray-200 p-1 bg-gray-50 mb-4">
              <TabsTrigger
                value={currentYear.toString()}
                className="flex-1 rounded-md py-1.5 text-sm data-[state=active]:bg-white data-[state=active]:text-gray-900"
              >
      ...
      {Object.keys(getHolidayTypeStats()).length > 0 && (
        ...
        >
          <div className="px-4 py-3 border-b border-gray-100 flex items-center">
            <FlagIcon className="h-4 w-4 mr-2 text-teal-600" aria-hidden="true" />
            <h2 id="holiday-types-heading" className="text-sm font-medium text-gray-900">
              Holiday Types
            </h2>
          </div>
              ...
              {Object.entries(getHolidayTypeStats()).map(([type, count]) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span className="ml-1 bg-white/70 rounded-full w-5 h-5 inline-flex items-center justify-center text-xs">
                      {count}
                    </span>
                  </div>
                );
      ...
      >
        <div className="px-4 py-3 border-b border-gray-100 flex items-center">
          <InfoIcon className="h-4 w-4 mr-2 text-teal-600" aria-hidden="true" />
          <h2 id="about-holidays-heading" className="text-sm font-medium text-gray-900">
        ...
        <div className="p-4 text-sm text-gray-600">
          ...
          <p className="mb-2">
            <span className="font-medium text-gray-700">Holiday planning tips:</span>
            {''}
      ...
      <div className="bg-gradient-to-r from-teal-50 to-blue-50 rounded-lg p-5 mt-8 mb-6 border-l-4 border-teal-500 shadow-sm relative overflow-hidden">
        {/* Decorative elements */}
        ...
        <div className="flex flex-col md:flex-row items-center text-left">
          <div className="flex-1 mb-4 md:mb-0 md:mr-6 relative z-10">
            <h3 className="text-base md:text-lg font-semibold text-teal-800 mb-2">
              Maximize Your Vacation Days with {location.country} Public Holidays {currentYear}-
              {nextYear}
            </h3>
            <p className="text-sm md:text-base text-teal-700 mb-3">
              This strategic holiday planner shows how to combine{''}
  ...
  if (Object.keys(holidaysByMonth).length === 0) {
    return (
      <div className="py-6 text-center">
        ...
        <p className="text-gray-500">No holidays found for this location and year.</p>
        <Link href="/holidays" className="inline-block mt-3 text-sm text-teal-600 hover:underline">
  ...
  return (
    <div className="space-y-6">
      {Object.entries(holidaysByMonth).map(([month, monthHolidays]) => (
        <div key={month} className="bg-gray-50 rounded-lg overflow-hidden">
          <div className="px-4 py-3 flex items-center border-b border-gray-200">
            <div className="w-8 h-8 rounded-md bg-teal-50 flex items-center justify-center mr-3">
              <span className="font-medium text-teal-600">{month.substring(0, 3)}</span>
            </div>
          ...
          <ul className="divide-y divide-gray-200" role="list">
            {monthHolidays.map((holiday, index) => {
              ...
              switch (holiday.type) {
                case 'public':
                  typeLabel = 'Public';
                  typeBadgeClass = 'bg-green-50 text-green-700 border-0';
                  break;
                case 'bank':
                  typeLabel = 'Bank';
                  typeBadgeClass = 'bg-blue-50 text-blue-700 border-0';
                  break;
                case 'school':
                  typeLabel = 'School';
                  typeBadgeClass = 'bg-amber-50 text-amber-700 border-0';
                  break;
                case 'optional':
                  typeLabel = 'Optional';
                  typeBadgeClass = 'bg-purple-50 text-purple-700 border-0';
                  break;
                default:
                  typeLabel = 'Observance';
                  typeBadgeClass = 'bg-gray-50 text-gray-700 border-0';
              }

              return (
                <li key={index} className="flex p-0" role="listitem">
                  <div className="w-16 flex-shrink-0 bg-white py-3 px-2 flex flex-col items-center justify-center border-r border-gray-200">
                    <div className="font-semibold text-gray-900">{dayNumber}</div>
                  ...
                  <div className="flex-1 p-3">
                    ...
                    <div className="flex items-center mt-1.5 space-x-1.5">
                      ...
                      {holiday.substitute && (
                        <span className="text-xs text-yellow-600 px-1.5 py-0.5 bg-yellow-50 rounded-sm">
                          Substitute
                        </span>
                      )}
                    </div>
