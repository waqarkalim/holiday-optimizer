...
import { cn, DayType, getDayTypeClasses } from '@/lib/utils';
...
export const BreakCard = ({ breakPeriod }: BreakCardProps) => {
  ...
  return (
    <div className="bg-white dark:bg-gray-800/50 rounded-lg shadow-sm ring-1 ring-gray-900/5 dark:ring-white/10 p-4">
      ...
      <div className="mt-3 grid grid-cols-4 gap-2 grow">
        ...
        <div className="flex space-x-0.5">
          {breakPeriod.days.map((day) => (
            <Tooltip key={day.date}>
              ...
              <TooltipContent>
                <p
                  className="text-xs">{`${format(parse(day.date, 'yyyy-MM-dd', new Date()), 'MMM d')} - ${day.isCTO ? 'CTO Day' :
